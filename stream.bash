...
#ffmpeg -listen 1 -i rtmp://"$IP":6645/stream/"$STREAMKEY" -c:v copy -c:a copy -flags +cgop -g 60 -hls_time 2 -hls_list_size 1 -hls_allow_cache 1 -hls_flags delete_segments -flush_packets 1 stream.m3u8 -f flv -vcodec libx264 "$TWITCHSRV""$TWITCHKEY" -f segment -c:v copy -c:a copy -strftime 1 -segment_time 1000000000 -segment_format mp4 'vods/%Y-%m-%d_%H-%M-%S_vod.mp4'; \cp -f done.m3u8 stream.m3u8;
